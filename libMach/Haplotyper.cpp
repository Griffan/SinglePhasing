...
void Haplotyper::OutputMLEs(Pedigree & ped, const String & prefix, bool mldetails)
   ...
   FILE * geno = fopen(prefix + ".mlgeno", "wt");
   FILE * info = fopen(prefix + ".mlinfo", "wt");
   FILE * qc   = mldetails ? fopen(prefix + ".mlqc", "wt") : NULL;
   FILE * prob = mldetails ? fopen(prefix + ".mlprob", "wt") : NULL;
   ...
   for (int i = 0; i < individuals - phased; i++)
      ...
      fprintf(dose, "%s->%s ML_DOSE", (const char *) ped[i].famid, (const char *) ped[i].pid);
      fprintf(geno, "%s->%s ML_GENO", (const char *) ped[i].famid, (const char *) ped[i].pid);

      if (qc)
         fprintf(qc, "%s->%s ML_QC", (const char *) ped[i].famid,  (const char *) ped[i].pid);

      if (prob)
         fprintf(prob, "%s->%s ML_PROB", (const char *) ped[i].famid, (const char *) ped[i].pid);
      ...
      for (int marker = 0; marker < markers; marker++)
         ...
         fprintf(dose, " %.3f", posterior[0][marker] * 2.0 + posterior[1][marker]);
         fprintf(geno, " %s/%s", (const char *) info->GetAlleleLabel((best + 2) / 2),
                                 (const char *) info->GetAlleleLabel((best + 3) / 2));

         if (qc)
            fprintf(qc, " %.3f", posterior[best][marker]);

         if (prob)
            fprintf(prob, " %.3f %.3f", posterior[0][marker], posterior[1][marker]);
